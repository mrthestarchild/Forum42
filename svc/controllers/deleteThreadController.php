...
class DeleteThreadController extends BaseModel
{
    public $token;
    public $userToken;
    public $threadId;
    ...
    public function deleteThread() : ResponseModel
    {
        ...
        if($check){
         ...
         if($this->dao->beginTransaction()){
            ...
            try{
                $updateThread = $this->dao->prepare("UPDATE thread SET is_active = 0 WHERE thread_id = :threadId");
                $updateThread->bindParam(':threadId', $this->threadId);
                $updateThread->execute();
                if ($this->dao->inTransaction()){
                    $this->dao->commit();
                    $response = new ResponseModel('SUCCESS','The thread has been updated in the database.',200);
                    return $response;
                }  
            }
